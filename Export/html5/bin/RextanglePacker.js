...
var Main = function() {
	this.mRectangles = new Array();
	this.mText = new openfl.text.TextField();
	this.mCopyRight = new openfl.text.TextField();
	this.mBitmapData = new openfl.display.BitmapData(480,480,true,-1);
	...
	var bitmap = new openfl.display.Bitmap(this.mBitmapData);
	this.addChild(bitmap);
	bitmap.set_x(15);
	bitmap.set_y(40);
	this.mCopyRight.set_x(15);
	this.mCopyRight.set_y(5.);
	this.mCopyRight.set_width(300);
	this.mCopyRight.set_text("Rectangle Packer");
	this.addChild(this.mCopyRight);
	this.mText.set_x(280);
	this.mText.set_y(5.);
	this.mText.set_width(200);
	this.addChild(this.mText);
	this.mScalingBox = new tools.ScalingBox(15,40,450,410);
...
Main.prototype = $extend(openfl.display.Sprite.prototype,{
	createRectangles: function() {
		...
		while(_g < 10) {
			var i = _g++;
			width = Std["int"](40. + Math.floor(Math.random() * 8) * 2.0 * 2.0);
			height = Std["int"](40. + Math.floor(Math.random() * 8) * 2.0 * 2.0);
			this.mRectangles.push(new openfl.geom.Rectangle(0,0,width,height));
		}
		var _g1 = 10;
		while(_g1 < 500) {
			var j = _g1++;
			width = Std["int"](6. + Math.floor(Math.random() * 8) * 2.0);
			height = Std["int"](6. + Math.floor(Math.random() * 8) * 2.0);
			this.mRectangles.push(new openfl.geom.Rectangle(0,0,width,height));
		}
	}
	,onAddedToStage: function(event) {
		...
		this.stage.align = openfl.display.StageAlign.TOP;
	}
	,onEnterFrame: function(event) {
		if(this.mScalingBox.get_boxWidth() != this.mScalingBox.get_newBoxWidth() || this.mScalingBox.get_boxHeight() != this.mScalingBox.get_newBoxHeight()) this.updateRectangles();
	}
	,updateRectangles: function() {
		var start = openfl.Lib.getTimer();
		var padding = 1;
		if(this.mPacker == null) this.mPacker = new utils.RectanglePacker(this.mScalingBox.get_newBoxWidth(),this.mScalingBox.get_newBoxHeight(),padding); else this.mPacker.reset(this.mScalingBox.get_newBoxWidth(),this.mScalingBox.get_newBoxHeight(),padding);
		var _g = 0;
		while(_g < 500) {
			var i = _g++;
			this.mPacker.insertRectangle(this.mRectangles[i].width | 0,this.mRectangles[i].height | 0,i);
		}
		this.mPacker.packRectangles();
		var end = openfl.Lib.getTimer();
		if(this.mPacker.get_rectangleCount() > 0) {
			this.mText.set_text(this.mPacker.get_rectangleCount() + " rectangles packed in " + (end - start) + "ms");
			this.mScalingBox.updateBox(this.mScalingBox.get_newBoxWidth(),this.mScalingBox.get_newBoxHeight());
			this.mBitmapData.fillRect(this.mBitmapData.rect,-1);
			var rect = new openfl.geom.Rectangle();
			var _g1 = 0;
			var _g2 = this.mPacker.get_rectangleCount();
			while(_g1 < _g2) {
				var j = _g1++;
				rect = this.mPacker.getRectangle(j,rect);
				this.mBitmapData.fillRect(new openfl.geom.Rectangle(rect.x,rect.y,rect.width,rect.height),-16777216);
				var index = this.mPacker.getRectangleId(j);
				var color = -15263997 + ((18 * ((index + 4) % 13) << 16) + (31 * (index * 3 % 8) << 8) + 63 * ((index + 1) * 3 % 5));
				this.mBitmapData.fillRect(new openfl.geom.Rectangle(rect.x + 1,rect.y + 1,rect.width - 2,rect.height - 2),color);
			}
		}
	}
	,__class__: Main
...
var tools = {};
tools.ScalingBox = function(x,y,width,height) {
	this.mDragging = false;
	this.mDragBox = new openfl.display.Sprite();
	this.mNewHeight = 0;
	this.mNewWidth = 0;
	this.mMaxHeight = 0;
	this.mMaxWidth = 0;
	...
	this.set_x(x);
	this.set_y(y);
	this.mDragBox.get_graphics().beginFill(16744528);
	this.mDragBox.get_graphics().drawCircle(0,0,10);
	this.mDragBox.get_graphics().endFill();
	this.addChild(this.mDragBox);
	this.mDragBox.addEventListener(openfl.events.MouseEvent.MOUSE_DOWN,$bind(this,this.onStartDrag));
	this.addEventListener(openfl.events.Event.ADDED_TO_STAGE,$bind(this,this.onAddedToStage));
	this.updateBox(width,height);
};
$hxClasses["tools.ScalingBox"] = tools.ScalingBox;
tools.ScalingBox.__name__ = ["tools","ScalingBox"];
tools.ScalingBox.__super__ = openfl.display.Sprite;
tools.ScalingBox.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,get_newBoxWidth: function() {
		return this.mNewWidth;
	}
	,get_newBoxHeight: function() {
		return this.mNewHeight;
	}
	,updateBox: function(width,height) {
		this.mWidth = width;
		this.mHeight = height;
		this.mNewWidth = width;
		this.mNewHeight = height;
		this.get_graphics().clear();
		this.get_graphics().lineStyle(1.0,0);
		this.get_graphics().drawRect(0,0,this.mWidth,this.mHeight);
	...
	,onMouseMove: function(event) {
		if(this.mDragging) {
			this.mNewWidth = event.stageX - this.mX | 0;
			this.mNewHeight = event.stageY - this.mY | 0;
			if(this.mNewWidth > this.mMaxWidth) this.mNewWidth = this.mMaxWidth;
			if(this.mNewHeight > this.mMaxHeight) this.mNewHeight = this.mMaxHeight;
		}
	}
	,onStartDrag: function(event) {
		this.mDragging = true;
	}
	,__class__: tools.ScalingBox
});
var utils = {};
utils.IntegerRectangle = function(x,y,width,height) {
	if(height == null) height = 0;
	if(width == null) width = 0;
	if(y == null) y = 0;
	if(x == null) x = 0;
...
$hxClasses["utils.IntegerRectangle"] = utils.IntegerRectangle;
utils.IntegerRectangle.__name__ = ["utils","IntegerRectangle"];
utils.IntegerRectangle.prototype = {
	__class__: utils.IntegerRectangle
};
utils.RectanglePacker = function(width,height,padding) {
	if(padding == null) padding = 0;
	this.mRectangleStack = new Array();
	this.mSortableSizeStack = new Array();
	this.mNewFreeAreas = new Array();
	this.mFreeAreas = new Array();
	this.mInsertedRectangles = new Array();
	this.mInsertList = new Array();
	this.mPackedHeight = 0;
	this.mPackedWidth = 0;
	this.mPadding = 8;
	this.mHeight = 0;
	this.mWidth = 0;
	this.mOutsideRectangle = new utils.IntegerRectangle(width + 1,height + 1,0,0);
	this.reset(width,height,padding);
};
$hxClasses["utils.RectanglePacker"] = utils.RectanglePacker;
utils.RectanglePacker.__name__ = ["utils","RectanglePacker"];
utils.RectanglePacker.prototype = {
	get_rectangleCount: function() {
	...
	,reset: function(width,height,padding) {
		if(padding == null) padding = 0;
	...
	,getRectangle: function(index,rectangle) {
		...
		return new openfl.geom.Rectangle(inserted.x,inserted.y,inserted.width,inserted.height);
	}
	,getRectangleId: function(index) {
		var inserted = this.mInsertedRectangles[index];
		return inserted.id;
	}
	,insertRectangle: function(width,height,id) {
		var sortableSize = this.allocateSize(width,height,id);
		this.mInsertList.push(sortableSize);
	}
	,packRectangles: function(sort) {
		if(sort == null) sort = true;
		if(sort) this.mInsertList.sort(function(a,b) {
			return Reflect.compare(a.width,b.width);
		});
		while(this.mInsertList.length > 0) {
			var sortableSize;
			sortableSize = js.Boot.__cast(this.mInsertList.pop() , utils.SortableSize);
			...
			if(index >= 0) {
				...
				if(target.right > this.mPackedWidth) this.mPackedWidth = target.right;
	...
	,filterSelfSubAreas: function(areas) {
		var i = areas.length - 1;
		while(i >= 0) {
			var filtered = areas[i];
			var j = areas.length - 1;
			while(j >= 0) {
				if(i != j) {
					var area = areas[j];
	...
	,generateNewFreeAreas: function(target,areas,results) {
		var x = target.x;
		var y = target.y;
		...
		while(i >= 0) {
			...
			if(!(x >= area.right || right <= area.x || y >= area.bottom || bottom <= area.y)) {
				if(targetWithPadding == null) targetWithPadding = this.allocateRectangle(target.x,target.y,target.width + this.mPadding,target.height + this.mPadding);
		...
		if(targetWithPadding != null && targetWithPadding != target) this.freeRectangle(targetWithPadding);
		this.filterSelfSubAreas(results);
	}
	,generateDividedAreas: function(divider,area,results) {
		var count = 0;
		...
		if(count == 0 && (divider.width < area.width || divider.height < area.height)) results[results.length] = area; else this.freeRectangle(area);
	}
	,getFreeAreaIndex: function(width,height) {
		var best = this.mOutsideRectangle;
		var index = -1;
		var paddedWidth = width + this.mPadding;
		var paddedHeight = height + this.mPadding;
		var i = this.mFreeAreas.length - 1;
		while(i >= 0) {
	...
	,allocateRectangle: function(x,y,width,height) {
		...
		return new utils.IntegerRectangle(x,y,width,height);
	}
	,freeRectangle: function(rectangle) {
		this.mRectangleStack[this.mRectangleStack.length] = rectangle;
	}
	,allocateSize: function(width,height,id) {
		...
		return new utils.SortableSize(width,height,id);
	}
	,freeSize: function(size) {
		this.mSortableSizeStack[this.mSortableSizeStack.length] = size;
	}
	,__class__: utils.RectanglePacker
};
utils.SortableSize = function(width,height,id) {
	this.width = width;
	this.height = height;
	this.id = id;
};
$hxClasses["utils.SortableSize"] = utils.SortableSize;
utils.SortableSize.__name__ = ["utils","SortableSize"];
utils.SortableSize.prototype = {
	__class__: utils.SortableSize
};
var $_, $fid = 0;
...
Main.Y_MARGIN = 40;
Main.BOX_MARGIN = 15;
Main.RECTANGLE_COUNT = 500;
Main.SIZE_MULTIPLIER = 2.0;
...
utils.RectanglePacker.VERSION = "1.3.0";
